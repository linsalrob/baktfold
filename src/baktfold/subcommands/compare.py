...
def subcommand_compare(
    ...
    ## copy the AA and 3Di from predictions directory if structures is false and baktfold compare is the command
    if structures is False:
    ...
    database_name = "all_baktfold_structures"

    if clustered_db:
        database_name = "all_baktfold_structures_clustered_searchDB"
    ...
    # NEEDS TO READ IN THE f"{prefix}_prostT5_3di_mean_probabilities.csv" - can't pass from the predict function in case using baktfold compare
    if predictions_dir is None: # if running baktfold run
        mean_probs_out_path: Path = (
            Path(output) / f"{prefix}_prostT5_3di_mean_probabilities.csv"
        )
    else: # if running baktfold compare or baktfold proteins-compare
        mean_probs_out_path: Path = (
