...
def get_genbank(genbank: Path) -> dict:
    ...
    logger.info(f"Checking if input {genbank} is a Bakta GenBank format file")
    def parse_records(handle):
        try:
            ...
            if "Bakta" in comment and "locus_tag" in cds_feature.qualifiers:
                logger.info(f"Detected Bakta style input GenBank. Using locus_tag qualifier from Bakta as the CDS IDs for baktfold.")
                method = "Bakta"
            else:
                logger.error(
                ...
                # code from baktfold - for now only support Bakta GenBanks
                # if "phrog" not in cds_feature.qualifiers and "protein_id" in cds_feature.qualifiers:
                #     logger.info(f"Detected NCBI Refseq style input Genbank. Using protein_id qualifier as the CDS IDs for baktfold.")
                #     method = "NCBI"
                # elif "phrog" in cds_feature.qualifiers and "ID" in cds_feature.qualifiers:
                #     logger.info(f"Detected Pharokka style input Genbank. Using ID qualifier from Pharokka as the CDS IDs for baktfold.")
                #     method = "Pharokka"
                # else:
                #     logger.error(
                #                 f"Feature {cds_feature} could not be parsed. Therefore, the input style format for {genbank} could not be detected. Please check your input."
...
def write_genbank(
    ...
    for record_id, record in gb_dict.items():
        ...
        if not proteins_flag:
            record.annotations["comment"] = f"Annotated with baktfold {version}.\nAnnotation Date: {format_time}.\nAnnotation Date: {format_time}.\nURL https://github.com/gbouras13/baktfold."
        ...
        for cds_feature in sorted_features:
            ...
            elif cds_feature.type == "tRNA": # trna features - fix the issue #76 https://github.com/gbouras13/baktfold/issues/76 
